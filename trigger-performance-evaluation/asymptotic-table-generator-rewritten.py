...
def exp_to_formula(e):
    ...
    if "trigger" in e:
        ...
        return r'$' + lhs + r'\: R_{' + interval + r'} \: B(x,y)$'
...
# df.drop(['rewritten real time', 'rewritten real time',
#         'rewritten sys time', 'rewritten sys time'], axis=1)
...
for experiment_name in experiments:
    ...
    for asym, exp in zip(['l', 'n', 'c'], dfs):
        x = exp.iloc[1:][['mean rewritten meval time']].reset_index(drop=True).div(exp.iloc[:-1][['mean rewritten meval time']].reset_index(drop=True))
        x.index = range(1,len(x)+1)

        exp[['factor rewritten meval time']] = x[['mean rewritten meval time']]
